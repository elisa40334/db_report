            ...
            if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<br><div id='unit-result'><a class='remove_line' href='employee.php?employee=" . $row["EName"] . "'>" . $row["EName"] . " 職位: " . ($row["position"] == "" ? "暫無資料" : $row["position"]) . " " . " 電話: " . ($row["EPhone"] == "" ? "暫無資料" : $row["EPhone"]) . " 工作内容: " . ($row["description"] == "" ? "暫無資料" : $row["description"]) . "</a></div>";
                    }
                } else {
                ...
                if ($result->num_rows > 0) {
                    // output data of each row
                    while ($row = $result->fetch_assoc()) {
                        echo "<br><div id='unit-result'>" . $row["EName"] . " 職位: " . $row["position"] . " " . " 電話: " . $row["EPhone"] . " 工作内容: " . $row["description"] . "</div>";
                    }
                } else {
                    echo "<br><div id='unit-result'>暫無資料</div>";
                }
